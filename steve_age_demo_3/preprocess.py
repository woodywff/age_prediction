...
def orgnize_files():
    '''
    *** (dataset specific) ***
    copy .nii from different centers folders to the target_path and rename them as <sub_id>.nii
    '''
    src_path = '/media/woody/Elements/ANAT_Normalize'
    dst_path = '/media/woody/Elements/Steve_age_data/ANAT_normalized'
    ...
    for i,folder in enumerate(folders):
        try:
            shutil.move(os.path.join(src_path,folder,'wsub-'+folder+'_ses-DS2_T1w.nii'),
                   os.path.join(dst_path,folder+'.nii'))
        except FileNotFoundError:
            try:
                shutil.move(os.path.join(src_path,folder,'wsub-'+folder+'_sesDS2_T1w.nii'),
                            os.path.join(dst_path,folder+'.nii'))
            except FileNotFoundError:
                try:
                    shutil.move(os.path.join(src_path,folder,'wsub-'+folder+'_ses-NFB2_T1w.nii'),
                                os.path.join(dst_path,folder+'.nii'))
                except FileNotFoundError:
                    try:
                        shutil.move(os.path.join(src_path,folder,'wsub-'+folder+'_ses-CLG2R_T1w.nii'),
                                    os.path.join(dst_path,folder+'.nii'))
                    except FileNotFoundError:
                        try:
                            shutil.move(os.path.join(src_path,folder,'wsub-'+folder+'_ses-CLG2_T1w.nii'),
                                        os.path.join(dst_path,folder+'.nii'))
                        except FileNotFoundError:
                            try:
                                shutil.move(os.path.join(src_path,folder,'wsub-'+folder+'_ses-DSA_T1w.nii'),
                                            os.path.join(dst_path,folder+'.nii'))
                            except FileNotFoundError:
                                try:
                                    shutil.move(os.path.join(src_path,folder,'wsub-'+folder+'_ses-CLG4R_T1w.nii'),
                                                os.path.join(dst_path,folder+'.nii'))
                                except FileNotFoundError:
                                    files = os.listdir(os.path.join(src_path,folder))
                                    found = False
                                    for file in files:
                                        re_result = re.match('^wco.*\.nii$',file)
                                        if re_result:
                                            shutil.move(os.path.join(src_path,folder,file),
                                                        os.path.join(dst_path,folder+'.nii'))
                                            found = True
                                    if not found:
