...
def preprocess_1(source_dir,target_dir_origin):
    ...
    for i,filename in enumerate(nii_list):
        re_result = re.match('^zReHoMap_sub-A.*\.nii$',filename)
        if re_result:
...
#                 np.save(target_filename,cropped_npy)
                npy_data = nib.load(os.path.join(source_dir,filename)).get_data()
                np.save(target_filename,npy_data)
...
def preprocess_main(mean_version=False):
    ...
    source_dir = '/media/woody/Elements/Steve_age_data/Reho'
    target_dir = './data_npy'
